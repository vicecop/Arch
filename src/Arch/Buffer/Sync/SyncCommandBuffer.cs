...
public sealed partial class SyncCommandBuffer : IDisposable
{
    ...
    public SyncCommandBuffer(int initialCapacity = 128)
    {
        ...
        Sets = new SyncSparseSet(initialCapacity);
        Adds = new SyncStructuralSparseSet(initialCapacity);
        Removes = new SyncStructuralSparseSet(initialCapacity);
    ...
    /// <summary>
    ///     Gets the amount of <see cref="Entity"/> instances targeted by this <see cref="SyncCommandBuffer"/>.
    /// </summary>
    public int Size { get; private set; }

    /// <summary>
    ///     All <see cref="Entity"/>'s created or modified in this <see cref="SyncCommandBuffer"/>.
    /// </summary>
    internal PooledList<Entity> Entities { get; set; }

    /// <summary>
    ///     A map that stores some additional information for each <see cref="Entity"/>, which is needed for the internal <see cref="SyncCommandBuffer"/> operations.
    /// </summary>
    internal PooledDictionary<int, BufferedEntityInfo> BufferedEntityInfo { get; set; }

    /// <summary>
    ///     All create commands recorded in this <see cref="SyncCommandBuffer"/>. Used to create <see cref="Entity"/>'s during <see cref="Playback"/>.
    /// </summary>
    ...
    public IEnumerable<Entity> GetCreatedEntities()
    {
        return Creates.Select(x => Entities[x.Index]);
    }
    ...
    public bool IsUpdated<T>(in Entity entity)
    {
        ...
        return Sets.Contains<T>(info.SetIndex);
    }
    ...
    public bool IsAdded<T>(in Entity entity)
    {
        ...
        return Adds.Contains<T>(info.AddIndex);
    }
    ...
    public bool IsRemoved<T>(in Entity entity)
    {
        ...
        return Removes.Contains<T>(info.RemoveIndex);
    }

    public bool IsDestroyed(in Entity entity)
    {
        if (!BufferedEntityInfo.TryGetValue(entity.Id, out var info))
        {
            return false;
        ...
        return Destroys.Contains(info.Index);
    }

    /// <summary>
    ///     Registers a new <see cref="Entity"/> into the <see cref="SyncCommandBuffer"/>.
    ...
    /// <param name="info">Its <see cref="BufferedEntityInfo"/> which stores indexes used for <see cref="SyncCommandBuffer"/> operations.</param>
    ...
    /// <summary>
    ///     Resolves an <see cref="Entity"/> originally either from a <see cref="SyncStructuralSparseArray"/> or <see cref="SyncSparseArray"/> to its real <see cref="Entity"/>.
    ...
    /// <summary>
    ///     Disposes the <see cref="SyncCommandBuffer"/>.
    /// </summary>
    public void Dispose()
    {
        Entities.Dispose();
...
public sealed partial class SyncCommandBuffer
{
    /// <summary>
